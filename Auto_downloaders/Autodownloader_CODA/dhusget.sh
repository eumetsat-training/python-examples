...
export DHUS_DEST="https://coda.eumetsat.int/"
...
while getopts ":d:u:p:l:P:q:C:m:i:t:s:e:S:E:f:c:T:o:V:h:F:R:D:r:O:N:L:n:Z:" opt; do
 case $opt in
	d)
...
if [ "${TO_DOWNLOAD}" == "manifest" -o "${TO_DOWNLOAD}" == "all" ]; then
    ...
    cat ${INPUT_FILE} | xargs -n 4 -P ${THREAD_NUMBER} sh -c ' while : ; do
        if [ "$PROD_VALUE" == "All" ]; then
            # get manifest
    	    echo "Downloading manifest ${3} from link ${DHUS_DEST}/odata/v1/Products('\''"$1"'\'')/Nodes('\''"$3".SEN3'\'')/Nodes('\'xfdumanifest.xml\'')/\$value";
	        ${WC} ${AUTH} ${TRIES} --progress=dot -e dotbytes=10M -c --output-file=./logs/log.${3}.log -O ./MANIFEST/manifest.safe-${3} "${DHUS_DEST}/odata/v1/Products('\''"$1"'\'')/Nodes('\''"$3".SEN3'\'')/Nodes('\'xfdumanifest.xml\'')/\$value" ;
            test=$?;
            if [ $test -eq 0 ]; then
                echo "Manifest successfully downloaded" ;
            fi;
            [[ $test -ne 0 ]] || break;
        else
            output_folder_stream=$output_folder/${3}'.SEN3'/
            mkdir -p $output_folder_stream
            # get product
            echo "Downloading product ${3} from link ${DHUS_DEST}/odata/v1/Products('\''"$1"'\'')/Nodes('\''"$3".SEN3'\'')/Nodes('\'$PROD_VALUE\'')/\$value";
            ${WC} ${AUTH} ${TRIES} --progress=dot -e dotbytes=10M -c --output-file=./logs/log.${3}.log -O $output_folder_stream'/'$PROD_VALUE "${DHUS_DEST}/odata/v1/Products('\''"$1"'\'')/Nodes('\''"$3".SEN3'\'')/Nodes('\'$PROD_VALUE\'')/\$value" ;
            test=$?;
            if [ $test -eq 0 ]; then
               echo "Product successfully downloaded" ;
            fi;
            [[ $test -ne 0 ]] || break;
        fi
    done '
