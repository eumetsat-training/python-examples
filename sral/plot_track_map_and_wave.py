...
# get waveforms
waveform_20_ku      = nc_fid.variables['waveform_20_ku'][:]
waveform_20_c       = nc_fid.variables['waveform_20_c'][:]
waveform_20_plrm_ku = nc_fid.variables['waveform_20_plrm_ku'][:]
...
print 'Num waveforms points 20_ku:      '+str(np.shape(waveform_20_ku)[0])
print 'Num waveforms points 20_c:       '+str(np.shape(waveform_20_c)[0])
print 'Num waveforms points 20_plrm_ku: '+str(np.shape(waveform_20_plrm_ku)[0])
...
m     = Basemap(projection=proj,llcrnrlat=-70,urcrnrlat=10,\
                llcrnrlon=150,urcrnrlon=170,lon_0=LON_0, resolution='i')
...
for ii in ocean_wv:
    count = count + 1
    if count <= 1:
        var = waveform_20_ku[ii,:]
        p1, =plt.plot(var/np.max(var),linewidth=2.0,color='#4682b4',zorder=20)
    elif np.mod(count,100)==0:
        var = waveform_20_ku[ii,:]
        plt.plot(var/np.max(var),linewidth=0.5,color='#4682b4',alpha=0.05,zorder=1)

plt.xlim([0, 128])
leg=plt.legend([p1],['Open ocean'])
leg.get_frame().set_linewidth(0.0)
plt.legend(frameon=False)

axes = plt.subplot(gs[2:4, 4:])
count = 0
for ii in sea_ice_wv:
    count = count + 1
    if count <= 1:
        var = waveform_20_ku[ii,:]
        p2, =plt.plot(var/np.max(var),linewidth=2.0,color='#8a2be2',zorder=20)
    elif np.mod(count,10)==0:
        var = waveform_20_ku[ii,:]
        plt.plot(var/np.max(var),linewidth=0.5,color='#8a2be2',alpha=0.05,zorder=1)

plt.xlim([0, 128])
plt.ylabel('Norm. waveform [count]',fontsize=fsz)
leg=plt.legend([p2],['Sea ice'])
leg.get_frame().set_linewidth(0.0)
plt.legend(frameon=False)

axes = plt.subplot(gs[4:, 4:])
count = 0
for ii in lead_wv:
    count = count + 1
    if count <= 1:
        var = waveform_20_ku[ii,:]
        p3, =plt.plot(var/np.max(var),linewidth=2.0,color='#3cb371',zorder=2)
    elif np.mod(count,10)==0:
        var = waveform_20_ku[ii,:]
        plt.plot(var/np.max(var),linewidth=0.5,color='#3cb371',alpha=0.05,zorder=1)

plt.xlim([0, 128])
plt.xlabel('Waveform echo index [bin]',fontsize=fsz)
leg=plt.legend([p3],['Lead'])
leg.get_frame().set_linewidth(0.0)
plt.legend(frameon=False)

plt.savefig('Track_Map_Wave.png')
